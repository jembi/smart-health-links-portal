...
import { mapEntityToModel, mapModelToEntity, mapModelToDto, mapDtoToModel } from "@/mappers/server-config-mapper";
...
describe("ServerConfig Mappers", () => {
  ...
  describe("mapEntityToModel", () => {
    it("should map a ServerConfigEntity to a ServerConfigModel", () => {
      ...
      expect(result?.getClientSecret()).toBe(serverConfigEntity.config_client_secret);
      expect(result?.getTokenEndpoint()).toBe(serverConfigEntity.config_token_endpoint);
      expect(result?.getRefreshTime()).toBe(serverConfigEntity.refresh_time);
      expect(result?.getAccessTokenResponse()).toBe(serverConfigEntity.access_token_response);
      expect(result?.getRefreshToken()).toBe(serverConfigEntity.config_refresh_token);
    });
  ...
  describe("mapDtoToModel", () => {
    it("should map a ServerConfigDto to a ServerConfigModel", () => {
      ...
      expect(result?.getAccessTokenResponse()).toBe(serverConfigDto.accessTokenResponse);
