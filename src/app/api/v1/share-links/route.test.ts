...
describe('API Route Handlers', () => {
  describe('POST /shlink', () => {
    it('should handle successful POST request', async () => {
      ...
      const request = new NextRequest('http://localhost/api/share-link', {
        method: 'POST',
  ...
  describe('GET /shlink', () => {
    it('should handle errors', async () => {
      const request = new NextRequest(
        'http://localhost/api/share-link?user_id=0',
        { method: 'GET' },
      );
      ...
      expect(response).toEqual(
        await handleApiValidationError(new Error('Database error')),
      );
    });
  });
});
