...
export async function GET(request: Request) {
  ...
  if (!userId)
    return NextResponse.json({ message: NOT_FOUND }, { status: 422 });

  try {
    const newShlink: SHLinkEntity[] = await getSHLinkUseCase(
      { repo },
      { user_id: userId },
    );
    return NextResponse.json(
      newShlink.map((shlink) => mapModelToMiniDto(mapEntityToModel(shlink))),
      { status: 200 },
    );
  } catch (error) {
    return handleApiValidationError(error);
