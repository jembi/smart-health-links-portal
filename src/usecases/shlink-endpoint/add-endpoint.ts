...
export const addEndpointUseCase = async (
  context: { repo: ISHLinkEndpointRepository },
  data: { endpoint: SHLinkEndpointModel },
): Promise<SHLinkEndpointModel> => {
  const oldSHLinkEndpoint = await context.repo.findOne({
    shlink_id: data.endpoint.getShlinkId(),
  });
