...
export const getSHLinkAccessesUseCase = async(context: { repo: ISHLinkAccessRepository }, data: { shlinkId: string }): Promise<SHLinkAccessModel[]> => {
    const shlinkAccesses = await context.repo.findMany({ shlink_id: data.shlinkId });
