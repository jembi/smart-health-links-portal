...
describe('searchPatientUseCase', () => {
    ...
    it('should return patient ID if patient data is found', async () => {
        ...
        const result = await searchPatientUseCase({ repo: mockRepo }, { patientId });
    ...
    it('should throw an error if configuration is missing', async () => {
        ...
        await expect(searchPatientUseCase({ repo: mockRepo }, { patientId: '12345' }))
            .rejects
    ...
    it('should throw an error if patient data is not found', async () => {
        ...
        await expect(searchPatientUseCase({ repo: mockRepo }, { patientId }))
            .rejects
            .toThrow(new ExternalDataFetchError('Patient Data not found.', 404));
    });
});
